...
conn = sqlite3.connect("./krut.db", check_same_thread=False)
cursor = conn.cursor()
...
#в get-запросе будет аргумент - port для соединения по сокету
#необходимо передать по нему всю информацию из БД в формате csv/txt
#строки таблицы разделяются \n а значение внутри одной строки запятой
conn.close() 
